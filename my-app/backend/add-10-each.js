...
// Start after last ISBN: 9789973244453 (487 total books)
let isbnCounter = 9973244453;
function genISBN() { isbnCounter++; return "978" + String(isbnCounter).padStart(10, "0"); }

async function add10Each() {
  try {
    ...
    // Add 10 books to each category (all categories)
    for (const category of categories) {
      const books = [];
      for (let i = 0; i < 10; i++) {
        books.push({
          title: `${category} Extra ${i + 1}`,
          author: `Extra Author ${i + 1}`,
          isbn: genISBN(),
          description: `Additional ${category.toLowerCase()} book`,
          category,
          subcategory: category === "Non-Fiction" ? "Self-Help" : category === "Academic" ? "History" : "General",
          publisher: "Extra Press",
          ...
          tags: [category.toLowerCase(), "extra"],
      ...
      added += books.length;
      console.log(`✅ Added 10 books to ${category}`);
    }

    // Add 10 books with subcategory "History"
    const historyBooks = [];
    for (let i = 0; i < 10; i++) {
      historyBooks.push({
        title: `History Extra ${i + 1}`,
        author: `Historian ${i + 1}`,
        isbn: genISBN(),
        description: "Additional history book",
        category: i < 5 ? "Non-Fiction" : "Academic",
        subcategory: "History",
        ...
        tags: ["history", "non-fiction", "extra"],
        rating: (Math.random() * 2 + 3).toFixed(1),
        ratingCount: Math.floor(Math.random() * 1000) + 100,
        borrowCount: Math.floor(Math.random() * 500) + 50,
        badge: "Popular",
    ...
    console.log(`✅ Added 10 History subcategory books`);

    // Add 10 books with subcategory "Self-Help"
    const selfHelpBooks = [];
    for (let i = 0; i < 10; i++) {
      selfHelpBooks.push({
        title: `Self-Help Extra ${i + 1}`,
        author: `Motivator ${i + 1}`,
        isbn: genISBN(),
        description: "Additional self-help book",
        category: i < 6 ? "Non-Fiction" : "Business",
        ...
        tags: ["self-help", "personal-growth", "extra"],
        rating: (Math.random() * 2 + 3).toFixed(1),
        ratingCount: Math.floor(Math.random() * 1000) + 100,
        borrowCount: Math.floor(Math.random() * 500) + 50,
        badge: "Trending",
    ...
    console.log(`✅ Added 10 Self-Help subcategory books`);
