  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => router.push("/library/reservations"), 1000);
      } else {
            ...
            <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap" }}>
              ...
              {(hasLocalPdf || hasRemoteUrl) && (
                <Button variant="contained" size="medium" startIcon={hasLocalPdf ? <MenuBookIcon /> : <OpenInNewIcon />}
                  onClick={() => { if (hasLocalPdf) router.push(`/library/books/${book._id}/read`); else window.open(book.sourceUrl, "_blank", "noopener,noreferrer"); }}
        ...
        {hasLocalPdf && (
          <Box sx={{ mt: 5, p: 4, bgcolor: "rgba(27,58,92,0.03)", borderRadius: "16px", textAlign: "center", border: "1px solid rgba(27,58,92,0.06)" }}>
            ...
            <Box sx={{ display: "flex", gap: 2, justifyContent: "center", flexWrap: "wrap" }}>
              <Button variant="contained" startIcon={<MenuBookIcon />} onClick={() => router.push(`/library/books/${book._id}/read`)}
