/**
 * Supabase Query Helpers (Frontend)
...
function queryUsers() { return new SupabaseQuery('users'); }
function queryBooks() { return new SupabaseQuery('books'); }
function queryBorrowings() { return new SupabaseQuery('borrowings'); }
function queryReservations() { return new SupabaseQuery('reservations'); }
function queryFines() { return new SupabaseQuery('fines'); }
function queryNotifications() { return new SupabaseQuery('notifications'); }
function queryContacts() { return new SupabaseQuery('contacts'); }
function queryWishlist() { return new SupabaseQuery('wishlist'); }
...
async function createMany(table, records) {
    const { data, error } = await supabase
        .from(table).insert(records).select();
    if (error) throw error;
    return data;
}
...
async function find(table, filters = {}, options = {}) {
    ...
    for (const [field, value] of Object.entries(filters)) {
        q = value !== null && value !== undefined ? q.eq(field, value) : q.filter(field, 'is', null);
    }
    if (options.orderBy) q = q.order(options.orderBy.field, { ascending: options.orderBy.ascending !== false });
    if (options.limit) q = q.limit(options.limit);
    if (options.offset) q = q.range(options.offset, options.offset + (options.limit || 1000) - 1);
...
async function update(table, filters, updateData) {
...
module.exports = {
    supabase,
    SupabaseQuery,
    queryUsers, queryBooks, queryBorrowings, queryReservations,
    queryFines, queryNotifications, queryContacts, queryWishlist,
    findById, findByIdAndUpdate, findByIdAndDelete,
    create, createMany, find, count, update, remove,
    searchBooks,
};
